...
class TrieRouterTest {
  ...
  void shouldExtractMultipleVariables() {
    ...
    // when
    final MatchResult result = router.findRoute("GET", "/api/users/noisif/orders/99");
    // then
    assertThat(result).isNotNull();
    assertThat(result.variables()).containsEntry("userId", "noisif").containsEntry("orderId", "99");
  }
