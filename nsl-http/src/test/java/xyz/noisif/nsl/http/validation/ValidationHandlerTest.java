 ...
 */
package xyz.noisif.nsl.http.validation;
...
import xyz.noisif.nsl.http.TestEnvelope;
import xyz.noisif.nsl.http.TestUser;
import xyz.noisif.nsl.http.validation.validator.LengthValidator;
import xyz.noisif.nsl.http.validation.validator.NotNullValidator;
import xyz.noisif.nsl.http.validation.validator.RangeValidator;
...
class ValidationHandlerTest {
  ...
  void setUp() {
    validationHandler =
        new ValidationHandler(
            Set.of(new NotNullValidator(), new LengthValidator(), new RangeValidator()));
  }
  ...
  void shouldPassValidation() {
    // given
    final TestUser validTestUser = new TestUser("NOISIF", 40);
    // when & then
  ...
  void shouldFailOnLength() {
    // given
    final TestUser invalidTestUser = new TestUser("NS", 20);
    // when & then
    assertThatThrownBy(() -> validationHandler.validate(invalidTestUser))
        .isInstanceOf(ValidationException.class)
  ...
  void shouldFailOnRange() {
    // given
    final TestUser invalidTestUser = new TestUser("NOISIF", 15);
    // when & then
    assertThatThrownBy(() -> validationHandler.validate(invalidTestUser))
        .isInstanceOf(ValidationException.class)
  ...
  void shouldWorkWithCache() {
    // given
    final TestUser testUser = new TestUser("NOISIF", 40);
    // when
  ...
  void shouldFailWhenNestedUserAgeIsInvalid() {
    // given
    final TestUser underageTestUser = new TestUser("NOISIF", 15);
  ...
  void shouldPassWhenEverythingIsValid() {
    // given
    final TestUser validTestUser = new TestUser("NOISIF", 25);
    final TestEnvelope validTestEnvelope = new TestEnvelope("REQ-103", validTestUser);
    // when & then
    assertThatCode(() -> validationHandler.validate(validTestEnvelope)).doesNotThrowAnyException();
  }
}
