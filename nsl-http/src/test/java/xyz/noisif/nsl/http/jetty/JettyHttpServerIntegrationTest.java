...
import xyz.noisif.nsl.http.TestConstants;
import xyz.noisif.nsl.http.TestEnvelope;
import xyz.noisif.nsl.http.TestUser;
import xyz.noisif.nsl.http.filter.CacheSpyFilter;
import xyz.noisif.nsl.http.header.TestHttpHeaderName;
import xyz.noisif.nsl.http.header.TestHttpHeaderValue;
...
public class JettyHttpServerIntegrationTest {
  ...
  void shouldReturnOkForValidRequest() throws Exception {
    // given
    final TestEnvelope payload = new TestEnvelope("REQ-001", new TestUser("NOISIF", 25));
    // when
    final HttpResponse<String> response = post(payload);
    // then
    assertThat(response.statusCode()).isEqualTo(HttpStatus.OK_200.getCode());
    assertThat(response.body()).contains("Success: NOISIF");
  }
  ...
  void shouldBypassFilterForUnannotatedRoute() throws Exception {
    ...
    assertThat(response.headers().firstValue(TestHttpHeaderName.X_SECURED_BY.getCode())).isEmpty();
  }
